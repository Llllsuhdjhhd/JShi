...
排序当前由**程序**按 `importance` 与近因（`last_used_at` / `created_at` / `use_count`）完成；**模型排序（情境相关性）占位未启用**。程序排序不改写长期字段。

### 7.3 容量比例

- 普通价值装载预算 = 活跃区字符上限 × 1/5（魔法书 `VALUE_LOAD_RATIO`）；
- 活跃区字符上限 = 模型窗口 × 1/30；工作集非保护字符预算 = **1500 字**（不随窗口放大，不管活跃区）。窗口默认 100 万 token。由 `14 统计分析与参数调优` 管理，不写死为稳定规则；
...
这是 **100 自己的条目整理**（去重、合并、升格建议），不是模块 300。300 产出的候选价值可以经本接口或 `review_value` 进入审核，两处不要写成同一个过程。
...
| 可调参数 | 普通价值预算（活跃区字符 × 1/5）、`personal_world_ratio`、`instance_max_length`（默认 800）、排序信号权重、`intervention_mode`（`open` / `reviewed` / `locked`） |
