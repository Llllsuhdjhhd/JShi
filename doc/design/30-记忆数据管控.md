...
- 接收 07 的活动收尾信号（编排层在 handoff 后调用 `run_once`）；
- 用自有游标 `memory_start` 收集 16 中尚未投递的经历（`list_experiences`）；
- 按**冲刷策略**（220）判断是否触发、触发到哪一批；
- 构造 `MemoryBatch`：按经历段映射 `MemoryExperience`（`text_raw → text`、`objects → objects`、`source_ids → source_ids`、`segment_id = 经历段 id`、`origin` 按 `output_kind`）；
...
| 参数 | 默认 | 说明 |
|------|------|------|
| `flush_max_chars` | 2000 | 未投递段 text 总字数 ≥ 阈值 → 冲刷一批 |
...
- `text`：`text_raw`，无原文的 state / silent 段用 `state_delta` 的 JSON；
- `objects`：经历段的对象映射表（**只透传**；说话人与提及对象须由输入侧放进映射表，00 §4.1）；
