...
直接打字后回车即发送。命令见 /help
主体 stone；对象 dp（长期）
闲时试验：180 秒无输入将主动开口，最多 2 轮后关闭。未切换说话人即默认当前对象在场。可用 /style 阿丘。
你：
```

在 `你：` 后打字，回车即走完整外部活动（识别 → 组装 → 认知 → 行动 → 收尾）。

本分支调试：约 3 分钟没有输入，会按当前说话人在场开一轮**闲时**（无对方原话，根据片场决定是否开口）。最多 2 轮后自动关闭，避免空耗 token。会话内任意输入会把计时清零。关掉本进程即作废，再开 `talk` 会再给 2 轮。环境变量 `JSHI_IDLE_DISABLE=1` 可关；`JSHI_IDLE_SECONDS` / `JSHI_IDLE_MAX_ROUNDS` 可改间隔与轮数。闲时试验建议 `/style 阿丘`（斯密斯底本，提示词里说明了闲时）；片场仍与斯密斯共用 `zone.json`，不是另一份现场。
...
| `/tool` `/tool now` `/tool list` `/tool plan` `/tool raw` `/tool <id>` | 只读工具过程。`/tool now` 看当前对象使用中与近一小时刚完成/失败的记挂（需求、描述、包装、最近反馈）；同一 need 拆成多本会并排列出。`/tool plan`（`/tool 205` 同义）看 205 策划材料。不调模型。 |
