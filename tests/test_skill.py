...
def test_cognition_skill_produces_usage_segments(tmp_path):
    ...
    assert resp.model == "deepseek-flash@v13"  # I-004：模型 + skill 版本
...
def test_instruction_keeps_rules_without_examples():
    ...
    assert "（未开口）" in text
    assert "合适时机告诉对方" in text or "合适时机再考虑是否告知" in text
    assert "【输出格式】" in text
...
def test_skill_model_port_routes_subject_activity_but_not_reflection():
    ...
    assert subject.model == "deepseek-flash@v13"
