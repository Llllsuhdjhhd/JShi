...
def test_suxipo_pack_content_and_contract():
    ...
    assert "unsaid" in instruction
    assert "（未开口）" in instruction
    assert "点名当前说话人" in instruction
    assert "正例" in instruction
    assert "反例" in instruction
    assert "留意时间、地点" in instruction
    assert "记下这块产生的时间" in instruction
    assert "[1年前]（lux）" in instruction
    assert "黄山好些一直雾蒙蒙" in instruction
    # 配置名不进提示词
...
def test_zone_store_boot_render_and_edit(tmp_path):
    ...
    rendered = zone.render("stone")
    assert rendered.startswith("B1[")          # 块号后跟 [时间]
    assert "我是匠石。" in rendered
    assert "路光把茶放到南窗桌上。" in rendered
    assert "分钟前]" in rendered  # 块带时间标签（1 小时内按分钟；精确时间存在块里）
    zone.apply_edit(
        "stone",
...
def test_zone_store_value_narration_capped_and_protected():
    ...
    rendered_v = zone.render("s")
    assert rendered_v.startswith("B1  " + "v" * 50)
    assert "块一" in rendered_v and "块二" in rendered_v
...
def test_smith_instruction_uses_shared_edit_rules():
    ...
    from jshi.style import AQIU

    assert "阿丘" not in instruction_for(AQIU)
    assert "斯密斯" not in instruction_for(AQIU)


def test_persona_time_labels_taught_on_reply_write_and_boot():
    from jshi.style import (
        SMITH,
        SUXIPO,
    ...
    for pack in (SUXIPO, SMITH):
        reply = reply_instruction_for(pack)
        write = write_instruction_for(pack)
        boot = boot_instruction_for(pack)
        assert "记下这块产生的时间" in reply
        assert "[1年前]（lux）" in reply
        assert "黄山好些一直雾蒙蒙" in reply
        assert "记下这块产生的时间" in write
        assert "[1年前]（lux）" in write
        assert "一年前的黄山之行" in write
        assert "不要自写块号和这个方括号" in write
        assert "时间戳" not in boot
        assert "相对现在的时间" in boot
        assert "行首方括号同样是发生时间" in boot
