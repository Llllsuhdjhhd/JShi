...
def test_format_speech_with_action_joins_verbal_and_embodied():
    from jshi.models import format_speech_with_action

    assert (
        format_speech_with_action("你好", "微微点头")
        == "你好（动作：微微点头）"
    )
    assert format_speech_with_action("", "转向声音来处") == "（动作：转向声音来处）"
    assert format_speech_with_action("", "无动作") == "（本轮未开口）"
    assert format_speech_with_action("嗨", "无动作") == "嗨"
...
def test_plan_and_context_do_not_create_activity(tmp_path):
    ...
    assert "/tool now" in tool_help.events[0].text
    assert "/tool plan" in tool_help.events[0].text
