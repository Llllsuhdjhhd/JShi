...
def _tool_engine():
    """按 ``JSHI_TOOL_ENGINE`` 选引擎；接 Pi 时把 provider / 模型 / key 一起给它。

    默认 **Pi**：工具要真的能办事，占位引擎只回显需求，把它当生产引擎会凭空
    「办成」根本不存在的事（2026-09-13 实测：占位引擎造出 weather_query 并回显
    当结果，包装写成「已经查到具体数值」）。要给测试或离线演示用占位引擎，
    显式设 ``JSHI_TOOL_ENGINE=stub``。

    可执行名也在这里兜底：Windows 上 npm 只装 ``pi.cmd``（没有无扩展名的 pi），
    用 ``JSHI_PI_EXECUTABLE`` 覆盖，默认 ``pi.cmd``（非 Windows 用 ``pi``）。
    """
    name = (os.getenv("JSHI_TOOL_ENGINE") or "pi").strip().lower()
    if name != "stub":
        from jshi.tool import PiEngine

        default_exe = "pi.cmd" if os.name == "nt" else "pi"
        return PiEngine(
            executable=(os.getenv("JSHI_PI_EXECUTABLE") or default_exe).strip(),
            provider=(os.getenv("JSHI_PI_PROVIDER") or "deepseek").strip(),
            model=(os.getenv("JSHI_MODEL_NAME") or "").strip(),
            api_key=(os.getenv("JSHI_MODEL_API_KEY") or "").strip(),
            home_dir=(os.getenv("JSHI_PI_HOME") or "").strip() or None,
        )
    return StubEngine()
...
def _tool_service(data_dir: Path, store: SkillConfigStore) -> ToolService:
    ...
    try:
        wrap_concurrency = max(
            1, int(os.getenv("JSHI_TOOL_WRAP_CONCURRENCY", "2"))
        )
    except ValueError:
        wrap_concurrency = 2
    return ToolService(
        ...
        wrap_concurrency=wrap_concurrency,
...
def _parser() -> argparse.ArgumentParser:
    ...
    introspect_idle = commands.add_parser(
        "introspect-idle", help="300：闲时回顾（入队并立刻跑一次）"
    )
    introspect_idle.add_argument("subject_id")
    introspect_idle.add_argument("--object-id", default=None)

    introspect_candidates = commands.add_parser(
        "introspect-candidates", help="300：列出自省候选（只读）"
    )
    introspect_candidates.add_argument("subject_id")
    introspect_candidates.add_argument("--limit", type=int, default=10)
    ...
    tool_log = commands.add_parser(
        "tool-log",
        help="查看工具过程（05 指示、200 交接、引擎反馈、交付片场、05 回写）",
    )
...
def main() -> None:
    ...
    elif args.command == "introspect-idle":
        _run_introspect_idle(process, args.subject_id, args.object_id)
    elif args.command == "introspect-candidates":
        _run_introspect_candidates(process, args.subject_id, args.limit)
...
def _drain_introspection(process, subject_id: str) -> None:
    """300：两拍之间把已入队的自省跑掉（不挡说话；失败不影响本轮）。"""
    runs = process.drain_introspection(subject_id, limit=1)
    for run in runs:
        print(
            f"[自省 {run.request.trigger.value}｜{run.answer.mode}"
            f"｜{run.duration_ms}ms]"
        )


def _run_introspect_idle(process, subject_id: str, object_id: str | None) -> None:
    request = process.introspect_idle(subject_id, object_id=object_id)
    if request is None:
        print("没有可回顾的条目（或命中冷却 / 已自省过）。")
        return
    print(f"已入队：{request.entry_ref}（{request.level.value}/{request.urgency.value}）")
    runs = process.drain_introspection(subject_id, limit=1)
    if not runs:
        print("队列里没有可执行的自省。")
        return
    for run in runs:
        print(run.answer.what or "（无）")
        print(
            f"[自省 {run.request.entry_ref}｜{run.answer.mode}"
            f"｜{run.duration_ms}ms｜{run.activity_id}]"
        )


def _run_introspect_candidates(process, subject_id: str, limit: int) -> None:
    """候选只读出口（设计 §5.16）：先把"记下来了"看得见，落点等 500 与审核。"""
    history = process.repository.list_history(subject_id, limit=500)
    rows = [
        item for item in history if item.event_type == "introspection_candidate"
    ]
    if not rows:
        print("暂无自省候选。")
        return
    for record in rows[-max(1, int(limit)):]:
        content = dict(record.content or {})
        kinds = "、".join(str(item) for item in content.get("suggested_kinds") or ())
        print(f"- {content.get('entry_ref')}｜{kinds}｜{record.created_at.isoformat()}")
        text = str(content.get("content") or "").replace("\n", " ")
        if text:
            print(f"  {text[:200]}")
...
def _run_tool_log(args) -> None:
    ...
    if getattr(args, "now", False):
        from jshi.tool.view import format_tool_now
        ...
        return
    if getattr(args, "turn", False):
        from jshi.subject import SubjectRepository
        from jshi.tool.view import format_turn_view

        repository = SubjectRepository(args.data_dir / "subject.sqlite3")
        marked = [
            (record.event_type, dict(record.content or {}))
            for record in repository.list_history(subject_id, limit=200)
            if record.event_type == "activity_response_marked"
        ]
        print(
            format_turn_view(
                service,
    ...
    if getattr(args, "plan", False):
        ...
        from jshi.experienceledger import SqliteExperienceLedger
        from jshi.style import StylePackStore, ZoneStore, is_persona
        from jshi.tool.scene import bind_scene_loader
        from jshi.tool.view import format_tool_plan_view

        ledger = SqliteExperienceLedger(args.data_dir / "subject.sqlite3")
        packs = StylePackStore(args.data_dir / "style_pack.json")
        pack_id = packs.get(subject_id) or ""
        kind = "persona" if is_persona(pack_id, registry=packs.registry) else "wood"
        print(
            format_tool_plan_view(
                service,
                subject_id=subject_id,
                object_id=object_id,
                item_id=getattr(args, "id", "") or "",
                scene_loader=bind_scene_loader(
                    InProcessActiveZone(ledger),
                    ZoneStore(args.data_dir / "zone.json"),
                ),
                zone_kind=kind,
                raw=bool(args.raw),
            )
