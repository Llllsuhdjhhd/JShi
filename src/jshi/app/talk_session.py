...
from jshi.models import STIMULUS_IDLE
...
IDLE_SECONDS_DEFAULT = 180
IDLE_MAX_ROUNDS_DEFAULT = 2
...
class IdleTrial:
    ...
    def __init__(
        ...
        *,
        ...
        if seconds is None:
            raw = os.environ.get("JSHI_IDLE_SECONDS", "").strip()
            seconds = float(raw) if raw else IDLE_SECONDS_DEFAULT
        if max_rounds is None:
            raw = os.environ.get("JSHI_IDLE_MAX_ROUNDS", "").strip()
            max_rounds = int(raw) if raw else IDLE_MAX_ROUNDS_DEFAULT
        if enabled is None:
            enabled = not _env_flag("JSHI_IDLE_DISABLE")
    ...
    def can_fire(self) -> bool:
        return self.enabled and self.fired < self.max_rounds

    def remaining(self) -> float | None:
        if not self.can_fire():
            return None
        return max(0.0, self.seconds - (time.monotonic() - self.last))
    ...
    def banner(self) -> str:
        return (
            f"闲时试验：{int(self.seconds)} 秒无输入将主动开口，"
            f"最多 {self.max_rounds} 轮后关闭。"
            "未切换说话人即默认当前对象在场。可用 /style 阿丘。"
        )
...
TALK_COMMANDS: tuple[tuple[str, str, str], ...] = (
    ...
    ("/tool", "now|list|plan|raw|id", "看工具；/tool now 看使用中与刚用完"),
    ("/login", "密码", "超级权限登录"),
    ("/logout", "", "退出超级权限"),
    ("/rule", "内容", "写入提示词【附加规则】"),
    ("/value", "内容", "写入个人世界：价值观"),
...
def format_speech_with_action(spoken: str, embodied: str) -> str:
    """兼容旧导入；实现见 ``jshi.models.format_speech_with_action``。"""
    from jshi.models import format_speech_with_action as _fmt

    return _fmt(spoken, embodied)
...
def format_turn_meta(
    *,
    ...
    return (
        f"[{mode}；{speaker_label}/{speaker_status}；"
...
class TalkSession:
    ...
    def handle_idle(self) -> TalkOutcome:
        """无对方原话的一轮外部活动。满调试轮数后不再开。"""
        if self.busy or not self.idle_trial.due():
            return TalkOutcome()
        outcome = self._handle_utterance("", stimulus=STIMULUS_IDLE)
        self.idle_trial.note_fire()
        events = (TalkEvent("notice", "—— 闲时 ——"), *outcome.events)
        if not self.idle_trial.can_fire():
            events = (
                *events,
                TalkEvent("notice", "闲时试验已关闭（已满 2 轮）。"),
            )
        return TalkOutcome(events, quit=outcome.quit)
    ...
    def _drain_introspection(self) -> None:
        """300：两拍之间把已入队的自省跑掉（不挡本轮，失败不影响对话）。"""
        drain = getattr(self.process, "drain_introspection", None)
        if not callable(drain):
            return
        try:
            drain(self.subject_id, limit=1)
        except Exception:
            pass

    def _handle_utterance(self, line: str, *, stimulus: str = "speech") -> TalkOutcome:
        ...
        try:
            ...
            if self._reply_streamed:
                # 语言+动作已在写场前由 on_reply 合成打印，此处不再补动作行。
                speech_text = ""
                self._reply_streamed = False
            else:
                speech_text = format_speech_with_action(spoken, embodied)
    ...
    def _tool_text(self, line: str) -> str:
        from jshi.style import is_persona
        from jshi.tool.scene import bind_scene_loader
        from jshi.tool.view import (
            format_tool_now,
        ...
        for token in parts[1:]:
            if token in ("plan", "205"):
                planning = True
            elif token in ("now", "current", "hot"):
                current = True
            elif token == "list":
                listing = True
            elif token == "raw":
                ...
                return (
                    "用法：/tool           当前对象最近一次过程\n"
                    "      /tool now       使用中与刚用完（描述、包装、最近反馈）\n"
                    "      /tool list      交接与记挂一览\n"
                    "      /tool plan      205 策划材料（含现读现场；/tool 205 同义）\n"
                    "      /tool raw       引擎反馈少截断（可与 now / plan 连用）\n"
        ...
        if planning:
            ...
            loader = getattr(planner, "scene_loader", None)
            if loader is None:
                loader = bind_scene_loader(
                    getattr(self.process, "active_zone", None),
                    getattr(self.process, "zone_store", None),
                )
            pack_id = ""
            packs = getattr(self.process, "style_packs", None)
            registry = getattr(packs, "registry", None) if packs is not None else None
            if packs is not None:
                pack_id = packs.get(self.subject_id) or ""
            kind = "persona" if is_persona(pack_id, registry=registry) else "wood"
            return format_tool_plan_view(
                service,
                subject_id=self.subject_id,
                object_id=object_id,
                item_id=item_id,
    ...
    def _prompt_parts(self) -> tuple[str, str] | str:
        ...
        try:
            ...
            persona_user_text = (
                self.process._persona_user_text(assembled, boot=boot, now=now)
