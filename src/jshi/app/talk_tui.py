...
class TalkApp(App[None]):
    ...
    def on_mount(self) -> None:
        ...
        if self.session.idle_trial.can_fire():
            self._write_chat(self.session.idle_trial.banner())
        self._picker().display = False
        self.query_one("#line", Input).focus()
        self.set_interval(1.0, self._tick_idle)

    def _tick_idle(self) -> None:
        if self._pending or self.session.busy or not self.session.idle_trial.due():
            return
        self._pending = True
        self.query_one("#status", Static).update("闲时…")

        def work() -> None:
            try:
                outcome = self.session.handle_idle()
    ...
    def _status_for(self, line: str) -> str:
        ...
        if line == "/context":
            return "正在预览上下文…"
        if line in {"/last", "/memory", "/memory_raw", "/tool"} or line.startswith("/tool "):
            return "正在读取…"
        if line.startswith("/"):
            return "处理中…"
        return "等待回应…"
